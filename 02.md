...
In this task you are configuring firewall rules in Azure portal, set up routes, associate subnets, and enable TLS inspection in the Firewall Policy. Obtain the Firewall's private IP and add a route to redirect traffic. Connect to a VM using Bastion, encounter blocked webpage errors, configure TLS inspection and application rules. Redirect traffic through the Firewall with defined rules for secure browsing.
