...
1. On the **Virtual HUB** page, in the left pane, select **Route Tables** under Routing section. The Route Tables page will populate the current route tables for this hub.
...
1. On the **Propagations** tab, select **default (1)** under Propagate routes from all branch connections to these labels across virtual WAN, select **vnet-shared-services2-<inject key="DeploymentID" enableCopy="false"/> (2)** under Virtual Networks section, then click on **Create (3)** to create the route table.
